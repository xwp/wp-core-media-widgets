...
class Test_WP_Widget_Image extends WP_UnitTestCase {
	...
	function test_update() {
		...
		$expected = array( 'size' => 'thumbnail' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid image size.
		$result = $widget->update( array( 'size' => 'big league' ), $instance );
		$this->assertSame( $result, $instance );

		// Should return valid image width.
		$expected = array( 'width' => 300 );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid image width.
		$result = $widget->update( array( 'width' => 'wide' ), $instance );
		$this->assertSame( $result, $instance );

		// Should return valid image height.
		$expected = array( 'height' => 200 );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid image height.
		$result = $widget->update( array( 'height' => 'high' ), $instance );
		$this->assertSame( $result, $instance );

		// Should return valid image alignment.
		$expected = array( 'align' => 'right' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid image alignment.
		$result = $widget->update( array( 'align' => 'next to caption' ), $instance );
		$this->assertSame( $result, $instance );

		// Should return valid image caption.
		$expected = array( 'caption' => 'A caption with <a href="#">link</a>' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid image caption.
		$result = $widget->update( array( 'caption' => '"><i onload="alert(\'hello\')" />' ), $instance );
		$this->assertSame( $result, $instance );

		// Should return valid alt text.
		$expected = array( 'alt' => 'A water tower' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid alt text.
		$result = $widget->update( array( 'alt' => '"><i onload="alert(\'hello\')" />' ), $instance );
		$this->assertSame( $result, array( 'alt' => '">' ) );

		// Should return valid link type.
		$expected = array( 'link_type' => 'file' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid link type.
		$result = $widget->update( array( 'link_type' => 'interesting' ), $instance );
		$this->assertSame( $result, $instance );

		// Should return valid link url.
		$expected = array( 'link_url' => 'https://example.org' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid link url.
		$result = $widget->update( array( 'link_url' => 'not_a_url' ), $instance );
		$this->assertNotSame( $result, $instance );
		$this->assertStringStartsWith( 'http://', $result['link_url'] );

		// Should return valid image classes.
		$expected = array( 'image_classes' => 'A water tower' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid image classes.
		$result = $widget->update( array( 'image_classes' => '"><i onload="alert(\'hello\')" />' ), $instance );
		$this->assertSame( $result, array( 'image_classes' => '">' ) );

		// Should return valid link classes.
		$expected = array( 'link_classes' => 'A water tower' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid link classes.
		$result = $widget->update( array( 'link_classes' => '"><i onload="alert(\'hello\')" />' ), $instance );
		$this->assertSame( $result, array( 'link_classes' => '">' ) );

		// Should return valid rel text.
		$expected = array( 'link_rel' => 'previous' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid rel text.
		$result = $widget->update( array( 'link_rel' => '"><i onload="alert(\'hello\')" />' ), $instance );
		$this->assertSame( $result, array( 'link_rel' => '">' ) );

		// Should return valid link target.
		$expected = array( 'link_target_blank' => false );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid  link target.
		$result = $widget->update( array( 'link_target_blank' => 'top' ), $instance );
		$this->assertSame( $result,$instance );

		// Should return valid image title.
		$expected = array( 'image_title' => 'What a title' );
		$result = $widget->update( $expected, $instance );
		$this->assertSame( $result, $expected );

		// Should filter invalid image title.
		$result = $widget->update( array( 'image_title' => '<h1>W00t!</h1>' ), $instance );
		$this->assertNotSame( $result, $instance );
